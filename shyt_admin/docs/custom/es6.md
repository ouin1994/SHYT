...
## 扩展运算符
__...__ 代表着扩展运算符或Rest(剩余)运算符,简单来说，就是将数组或者对象从整体展开。
```js
var arr = [12,5,8];
var arr2 = [...arr]
arr2.pop(); // 删除数组最后一项最后
...
## 箭头函数
箭头函数是ES6新增的函数声明方式，更加简化了函数的声明写法，与ES5的声明方式所声明的函数的最大区别在于它的this指向在函数声明时就已经固定，箭头函数的this指向与它外部一致。
```js
const fn = function (v) {
  return v;
};
...
## 函数的默认参数
在ES6中可以在函数声明时给函数的参数设置默认值，如果有多个参数，只能设置最后一个参数的默认值，设置了默认值的参数可以不用在使用时传值。
```js
// 给y设置了默认值可以不用传值
let add = (x,y=1)=>x+y
add(2) // 3 
...
```
[进阶用法1](https://www.jianshu.com/p/063f7e490e9a)
[进阶用法2](https://www.jianshu.com/p/1b63a13c2701)
